...
const BusinessSelect: React.FC<BusinessSelectProps> = ({ businesses, onChange, value }) => {
  return (
    <select
      ...
      value={value}
    >
      <option value="" className="bg-background">Selecciona un negocio</option>
      {businesses.length > 0 ? (
        ...
        <option value="" className="text-sm text-custom-textGris italic pl-2 bg-background">No hay negocios creados</option>
      )}
    </select>
  );
};
